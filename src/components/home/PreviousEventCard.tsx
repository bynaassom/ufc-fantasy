...
export default function PreviousEventCard({ event, performance }: { event: Event; performance: PreviousEventPerformance }) {
  return <Link href={`/historico/${event.slug}`} aria-label={`Abrir resultado de ${event.name}`} className="event-rail-card group block overflow-hidden border border-[var(--border)] bg-[var(--bg-card)]">
    <div className="relative aspect-[16/9] overflow-hidden bg-[var(--hero-ink)]"><EventBannerMedia event={event} alt="" sizes="(max-width: 767px) 82vw, (max-width: 1023px) 44vw, 360px" className="object-cover opacity-85 transition-transform duration-500 group-hover:scale-[1.04]" />{event.is_bonus && <div className="absolute left-3 top-3"><EventBonusLabel overlay /></div>}</div>
