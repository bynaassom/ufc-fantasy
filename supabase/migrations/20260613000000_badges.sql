...
CREATE POLICY "badges_insert_admin" ON badges FOR INSERT WITH CHECK (false);
CREATE POLICY "badges_update_admin" ON badges FOR UPDATE USING (false);
CREATE POLICY "badges_delete_admin" ON badges FOR DELETE USING (false);
...
INSERT INTO badges (slug, name, description, category, icon_name, tier, sort_order) VALUES
  ...
  ('viciado', 'Viciado', 'Participe de 25 eventos', 'volume', 'flame', 3, 3),
  ('mira_sniper', 'Mira de Sniper', 'Acerte 3 perfect picks em um único evento', 'accuracy', 'crosshair', 2, 4),
  ('mestre_palpites', 'Mestre dos Palpites', 'Acumule 10 perfect picks no total', 'accuracy', 'star', 2, 5),
  ('vidente', 'Vidente', 'Alcance 70%+ de precisão nos picks', 'accuracy', 'eye', 3, 6),
  ('em_frente', 'Em Frente', 'Mantenha uma sequência de 3 eventos melhorando de posição', 'streak', 'trending-up', 2, 7),
